...
export default function TombolaControls({ setAlert }) {
  const { tombola, setTombola, possibleNumbers, setPossibleNumbers } = useTombola();
  ...
  return (
    <div className="top-tabellone">
      <button className="button extract-button" type="button" onClick={handleClick}>
        Estrai
      </button>
      ...
      {lastExtracted && (
        <span className="last-extracted">
          Ultimo estratto: <span className="extracted-number">{lastExtracted}</span>
        </span>
      )}
    </div>
  );
